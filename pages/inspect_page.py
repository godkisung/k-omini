...
import qa_visualizer.drawing
import importlib
importlib.reload(qa_visualizer.drawing)
from qa_visualizer.drawing import draw_annotations_on_image
...
from qa_visualizer.analytics import detect_outliers
...
def _render_content_tab(selected_det):
    ...
    if hasattr(selected_det, 'html') and selected_det.html:
        with st.expander("📊 HTML 테이블", expanded=True):
...
def _render_main_visual_ui(doc, filtered_anns, filtered_indices, current_filtered_idx):
    ...
    if os.path.exists(full_image_path):
        ...
        annotated_image = draw_annotations_on_image(image.copy(), [ann.data for ann in doc.layout_dets], highlight_id=selected_det.anno_id)
...
def _render_validation_tab(selected_det, doc):
    val_results = run_validation(selected_det.data, doc.raw_data)
...
def _filter_annotations(doc, category_filter, show_only_errors):
    ...
    for i, ann in enumerate(doc.layout_dets):
        ...
        if show_only_errors:
            val_results = run_validation(ann.data, doc.raw_data)
...
def _render_ocr_validation_section(selected_det, doc):
    """OCR 기반 검증 결과를 표시합니다."""
    from qa_visualizer.ocr_utils import PADDLEOCR_AVAILABLE
    
    st.markdown("### 🤖 OCR 자동 검증")
    
    # PaddleOCR 비활성화 체크
    if not PADDLEOCR_AVAILABLE:
        st.error("⚠️ PaddleOCR이 설치되지 않았습니다. OCR 기능을 사용할 수 없습니다.")
        st.info("💡 설치 방법: `conda run -n base pip install paddlepaddle paddleocr`")
        return
    ...
    if selected_det.text and selected_det.text.strip():
        with st.spinner("OCR 텍스트 검증 중..."):
            ...
            if ocr_results:
                ...
                st.success("✅ OCR 텍스트 검증 통과")
    
    # 표 구조 검증 (표인 경우)
    if selected_det.category_type == "table" and hasattr(selected_det, 'html') and selected_det.html:
        with st.spinner("표 구조 검증 중..."):
            table_results = validate_table_structure_with_ocr(
                selected_det.data,
                full_image_path,
                config.TABLE_STRUCTURE_TOLERANCE
            )
